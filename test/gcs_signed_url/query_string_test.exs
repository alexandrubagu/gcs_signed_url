defmodule GcsSignedUrl.QueryStringTest do
  ...
  describe "create/6" do
    test "creates query string according to specs" do
      ...
      query_string = MUT.create(client, "some-credential-scope", iso_date_time, headers , 500, foo: "bar")
      assert "X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=project%40gcs_signed_url.iam.gserviceaccount.com%2Fsome-credential-scope&X-Goog-Date=some-date-time&X-Goog-Expires=500&X-Goog-SignedHeaders=some-signed-headers&foo=bar" == query_string
    end
  end
end
